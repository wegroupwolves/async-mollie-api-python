...
def main():
    try:
        ...
        params = {"limit": amount_of_customers_to_retrieve}
        ...
        if not len(customers):
            body += (
                "<p>You have no customers. You can create one from the examples.</p>"
            )
            return body

        body += "<p>Showing the last {num} customers for your API key.</p>".format(
            num=len(customers)
        )
