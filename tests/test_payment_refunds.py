...
def test_get_refund(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/payments/%s/refunds/%s" % (PAYMENT_ID, REFUND_ID),
        "refund_single",
    )
    ...
    assert (
        refund.description
        == "Required quantity not in stock, refunding one photo book."
    )
...
def test_create_refund(client, response):
    ...
    response.post(
        "https://api.mollie.com/v2/payments/%s/refunds" % PAYMENT_ID, "refund_single"
    )
...
def test_cancel_refund(client, response):
    ...
    canceled_refund = client.payment_refunds.with_parent_id(PAYMENT_ID).delete(
        REFUND_ID
    )
