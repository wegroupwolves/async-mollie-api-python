...
def test_get_order(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/orders/{order_id}/shipments".format(
            order_id=ORDER_ID
        ),
        "shipments_list",
    )
    response.get(
        "https://api.mollie.com/v2/orders/{order_id}/refunds".format(order_id=ORDER_ID),
        "order_refunds_list",
    )
    ...
    assert (
        order.checkout_url == "https://www.mollie.com/payscreen/order/checkout/pbjz8x"
    )
...
def test_cancel_order_lines(client, response):
    ...
    response.delete(
        "https://api.mollie.com/v2/orders/{order_id}/lines".format(order_id=ORDER_ID),
        "empty",
        204,
    )
