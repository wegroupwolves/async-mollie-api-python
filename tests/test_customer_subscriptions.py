...
def test_list_customer_subscriptions(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/customers/%s/subscriptions" % CUSTOMER_ID,
        "subscriptions_list",
    )
...
def test_get_customer_subscription_by_id(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/customers/%s/subscriptions/%s"
    ...
    subscription = client.customer_subscriptions.with_parent_id(CUSTOMER_ID).get(
        SUBSCRIPTION_ID
    )
...
def test_cancel_customer_subscription(client, response):
    ...
    subscription = client.customer_subscriptions.with_parent_id(CUSTOMER_ID).delete(
        SUBSCRIPTION_ID
    )
    assert isinstance(subscription, Subscription)
...
def test_create_customer_subscription(client, response):
    ...
    subscription = client.customer_subscriptions.with_parent_id(CUSTOMER_ID).create(
        data=data
    )
    assert isinstance(subscription, Subscription)
    assert subscription.id == SUBSCRIPTION_ID
...
def test_update_customer_subscription(client, response):
    ...
    subscription = client.customer_subscriptions.with_parent_id(CUSTOMER_ID).update(
