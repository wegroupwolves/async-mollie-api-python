...
def test_create_customer(client, response):
    ...
    customer = client.customers.create(
        {"name": "Customer A", "email": "customer@example.org", "locale": "nl_NL"}
    )
...
def test_update_customer(client, response):
    ...
    response.patch(
        "https://api.mollie.com/v2/customers/%s" % CUSTOMER_ID, "customer_updated"
    )

    updated_customer = client.customers.update(
        CUSTOMER_ID,
        {"name": "Updated Customer A", "email": "updated-customer@example.org"},
    )
...
def test_get_customer(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/customers/%s/payments" % CUSTOMER_ID,
        "customer_payments_multiple",
    )
...
def test_customer_get_related_mandates(client, response):
    ...
    response.get(
        "https://api.mollie.com/v2/customers/%s/mandates" % CUSTOMER_ID,
        "customer_mandates_list",
    )
